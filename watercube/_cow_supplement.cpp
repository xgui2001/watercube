...
template <typename T>
void sbuff_push_back(StretchyBuffer<T> *buffer, T element)
{
...
template <typename T>
void sbuff_free(StretchyBuffer<T> *buffer)
{
    buffer->length = 0;
    buffer->capacity = 0;
    if (buffer->data)
...
void fps_camera_move(FPSCamera *human)
{
    vec3 ds = {};
    {
        if (input.key_held['w'])
        {
            ds += transformVector(RotationY(human->theta), V3(0, 0, -1));
        }
        if (input.key_held['s'])
        {
            ds += transformVector(RotationY(human->theta), V3(0, 0, 1));
        ...
        if (input.key_held['a'])
        {
            ds += transformVector(RotationY(human->theta), V3(-1, 0, 0));
        }
        if (input.key_held['d'])
        {
            ds += transformVector(RotationY(human->theta), V3(1, 0, 0));
...
{
    if (widget_active_widget_ID != 0 && widget_active_widget_ID != WIDGET_ID_TRANSLATE)
        return;
    ...
    if (!input.mouse_left_held)
        STILL_HOLDING_MOUSE_AFTER_SELECTING_POINT = false;
