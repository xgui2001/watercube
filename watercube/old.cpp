...
void world()
{
    ...
    for (int i = 0; i < 400; i++)
    {
        point[i] = {random_position() / 100, i * 0.02};
    }
    ...
    */
    ...
    char *frag = R""""(
      ...
      vec2 p = fmod(uv*3.14, 3.14)-200.0;
    float c = 1.0;

    vec2 i = p;
    for (int n = 0; n < 3; n++)
    {
        i = p + V2(cos(iTime - i.x) + sin(iTime + i.y), sin(iTime - i.y) + cos(iTime + i.x));
        c += 1.0/norm(V2(p.x / (sin(i.x+iTime)/.006),p.y / (cos(i.y+iTime)/.006)));
    }
    c /= float(3);
    c = 1.17-pow(c, 1.4);
    vec3 color = vec3(pow(c, 5.0));
    color = clamp(color + vec3(0.1, 0.5, 0.6), 0.0, 1.0);
    ...
    */
    ...
    while (begin_frame())
    {
        ...
        vec2 particle_normalized[400][400] = {};
        double particle_weight[400] = {};
        double particle_pressure[400] = {};

        // vec2 mouse_displacement[400] = {};
        //  vec2 tensile_s[500] = {};
        //  vec2 tensile_A[500] = {};
        //  vec2 tensile_B[500] = {};
        ...
        for (int i = 0; i < int(drawings.length); i++)
        {
        ...
        */
        ...
        for (int i = 0; i < num; i++)
        {
            speed[i] -= h * gravity * 1 / 100;
            // speed[1] -= h * gravity * 1 / 100;
            ...
            for (int j = 0; j < num; j++)
            {
                if (i != j) // cannot be while (i != j)
                {
                    ...
                    if (D < 1 && !IS_ZERO(D))
                    {
                        ...
                        speed[i] += h * 0.05 * (particle_pressure[i] + particle_pressure[j]) * particle_weight[i] * particle_normalized[i][j];
                        speed[j] += h * 0.05 * (particle_pressure[j] + particle_pressure[i]) * particle_weight[j] * particle_normalized[j][i];
                        ...
                        // viscosity
                        speed[i] += h * 0.001 * (speed[i] - speed[j]);
                        speed[j] += h * 0.001 * (speed[j] - speed[i]);

                        // tensile

                        // printf("%f ", speed[0]); // getting nan?
                        // printf("s ");
                        // printf("%f ", speed[1]); // getting nan?
                    }
                }
            }
        ...
        // basic_draw(POINTS, PV, 1, &point[i], monokai.blue, 20.0);
        basic_draw(primitive_types[primitive_num], PV, num, point, monokai.blue, size);
        ...
        */
        ...
        // fancy_draw(P, V, Identity4x4,
        // 2, triangle_indices, 4, duck_vertex_positions,
        // NULL, NULL, {},
        // vertex_texCoords, "pixil-frame-0.png");
    }
}
